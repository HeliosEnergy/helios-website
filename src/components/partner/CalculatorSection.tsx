        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
          <div>
            ...
            <div className="relative">
              <span className="absolute left-3 top-1/2 transform -translate-y-1/2 text-white/60 text-lg font-medium">
                $
              </span>
              <input
                type="number"
                ...
                className="w-full pl-8 pr-4 py-3 border-2 border-[#fbbf24] rounded-lg text-lg focus:outline-none focus:border-[#f59e0b] bg-white/10 text-white font-medium"
              />
            </div>
          </div>
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-4 py-3 border-2 border-[#fbbf24] rounded-lg text-lg focus:outline-none focus:border-[#f59e0b] bg-white/10 text-white font-medium"
            >
              <option value="250 kW" className="bg-black">250 kW</option>
              <option value="500 kW" className="bg-black">500 kW</option>
              <option value="750 kW" className="bg-black">750 kW</option>
              <option value="1 MW" className="bg-black">1 MW</option>
              <option value="1.5 MW" className="bg-black">1.5 MW</option>
              <option value="2 MW" className="bg-black">2 MW</option>
            </select>
          </div>
        </div>

        {/* Chart */}
        <div className="bg-white/5 border border-white/10 rounded-lg p-4 md:p-8 shadow-sm overflow-hidden">
          <div className="mb-6">
            <h3 className="text-lg font-semibold text-white/60 mb-4 text-center">
            ...
            <div className="flex justify-center gap-6 mb-6">
              ...
              <div className="flex items-center">
                ...
                <span className="text-sm font-medium text-white">Helios</span>
              </div>
            </div>
          </div>
          ...
          <div className="relative overflow-x-auto">
            {/* Y-axis label */}
            <div className="absolute -left-16 md:-left-20 top-1/2 transform -translate-y-1/2 -rotate-90 text-sm font-medium text-white/60">
              Gross Income ($)
            </div>

            {/* Y-axis ticks */}
            <div className="absolute left-0 h-64 md:h-80 flex flex-col justify-between text-xs text-white/50 pr-2">
            ...
            <div className="ml-12 md:ml-16 pl-4">
              <div className="flex items-end justify-between h-64 md:h-80 border-l-2 border-b-2 border-white/30 min-w-[600px]">
                  ...
                  return (
                    <div key={index} className="flex-1 flex justify-center items-end gap-1 px-0.5 md:px-1">
                      <div className="flex flex-col items-center">
                        {/* Grid bar */}
                        <div className="text-xs font-semibold mb-1 text-center text-white">
                          {formatCurrency(data.grid)}
                        </div>
                        <div
                          className="w-4 md:w-6 bg-white"
              ...
              {/* X-axis labels */}
              <div className="flex justify-between mt-2 text-xs text-white/60 min-w-[600px]">
                {chartData.map((data, index) => (
            ...
            <div className="text-center mt-4">
              <span className="text-sm font-medium text-white/60">Timeline</span>
            </div>
          </div>
          ...
          <div className="mt-8 text-center">
            <p className="text-lg text-white/70">
              Annual revenue increase from Helios: <span className="text-3xl font-normal text-white">{formatCurrency(annualIncrease)}</span>
            </p>
          </div>
        </div>
      </section>
